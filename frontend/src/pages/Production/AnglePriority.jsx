...
import { Plus, Search, FileText, Calendar, Edit2, Trash2, ClipboardList, Eye, Loader2, X, Download } from 'lucide-react';
import ProductionPriorityForm from '../../components/forms/ProductionPriorityForm';
import { priorityAPI } from '../../services/api';

export default function AnglePriority() {
  ...
  const moduleType = 'ANGLE';
  const tabs = ["Angle master", "ironworker", "PEDDINGHAUS DRILL LINE (PCS. LARGER THAN 6')"];
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-slate-900">Angle Priority</h2>
          <p className="text-sm text-slate-500 mt-0.5">Manage angle production priorities and tracking</p>
        </div>
        <button onClick={openAdd} className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-amber-500 to-orange-500 text-white text-sm font-bold shadow-lg shadow-amber-500/20 hover:from-amber-400 hover:to-orange-400 transition-all hover:shadow-xl transform hover:scale-[1.02]">
      ...
      <div className="relative max-w-md">
        ...
        <input type="text" placeholder="Search by schedule or process..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-9 pr-4 py-2.5 rounded-xl border border-slate-200 bg-white text-sm outline-none focus:border-amber-400 focus:ring-4 focus:ring-amber-500/5 transition-all" />
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-3xl border border-slate-200/60 p-16 text-center shadow-sm">
          <div className="w-16 h-16 bg-slate-50 rounded-2xl flex items-center justify-center mx-auto mb-4"><FileText className="w-8 h-8 text-slate-200" /></div>
          <h3 className="text-lg font-bold text-slate-800 mb-1">No records found</h3>
          <p className="text-sm text-slate-500">Click "New Record" to start adding priorities.</p>
        </div>
      ) : (
            ...
            <div key={record.id} className="bg-white rounded-3xl border border-slate-100 p-6 shadow-sm hover:shadow-xl transition-all group relative border-t-4 border-t-amber-500">
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 bg-amber-50 rounded-2xl flex items-center justify-center text-amber-600 shadow-sm"><ClipboardList className="w-6 h-6" /></div>
                  <div>
                ...
                <div className="flex gap-1 no-print">
                   <button onClick={() => openView(record)} className="p-2 rounded-lg text-slate-400 hover:text-amber-600 hover:bg-amber-50 transition-all" title="View Plan"><Eye className="w-4 h-4" /></button>
                   <button onClick={() => openEdit(record)} className="p-2 rounded-lg text-slate-400 hover:text-blue-600 hover:bg-blue-50 transition-all" title="Edit Record"><Edit2 className="w-4 h-4" /></button>
                   <button onClick={() => handleDelete(record.id)} className="p-2 rounded-lg text-slate-400 hover:text-red-500 hover:bg-red-50 transition-all" title="Delete Record"><Trash2 className="w-4 h-4" /></button>
                </div>
              </div>
              <div className="space-y-3 mb-2 bg-slate-50/50 p-4 rounded-2xl border border-slate-100">
                <div className="flex items-center justify-between text-xs"><span className="font-bold text-slate-400 uppercase tracking-tighter">Rate:</span><span className="font-bold text-slate-700">{record.rate} lbs/day</span></div>
                <div className="flex items-center justify-between text-xs"><span className="font-bold text-slate-400 uppercase tracking-tighter">Items:</span><span className="font-bold text-slate-700">{record.items?.length || 0} Rows</span></div>
              </div>
            </div>
          ))}
        </div>
