...
export default function UserAccess() {
  ...
  const handleSelectAllModules = () => {
    setFormData(prev => ({ ...prev, allowed_modules: MODULE_CONFIG.map(m => m.key) }));
  };

  const handleDeselectAllModules = () => {
    setFormData(prev => ({ ...prev, allowed_modules: [] }));
  };

  const openEditModal = (user) => {
    setSelectedUser(user);
    let userModules = user.allowed_modules;
    if (!userModules || userModules.length === 0 || user.role === 'admin') {
      userModules = MODULE_CONFIG.map(m => m.key);
    }
    setFormData({
  ...
  const getRoleLabel = (role) => {
    ...
    const opt = roleOptions.find(o => o.value === role.toLowerCase());
    return opt ? opt.label : (role.charAt(0).toUpperCase() + role.slice(1));
  };
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 rounded-xl shadow-2xl w-full max-w-xl max-h-[90vh] flex flex-col overflow-hidden animate-scale-up">
            ...
            {/* Form */}
            <form onSubmit={handleAddUserSubmit} className="p-6 space-y-4 flex-1 overflow-y-auto">
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-slate-100 mt-6">
                <button
                  type="button"
                  onClick={() => setShowAddModal(false)}
                  className="px-4 py-2 border border-slate-200 text-slate-600 hover:bg-slate-50 rounded-xl font-bold text-xs transition-all"
                >
                  Cancel
                </button>
                <button
      ...
      {showEditModal && selectedUser && (
        <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 rounded-xl shadow-2xl w-full max-w-xl max-h-[90vh] flex flex-col overflow-hidden animate-scale-up">
            ...
            {/* Form */}
            <form onSubmit={handleEditUserSubmit} className="p-6 space-y-4 flex-1 overflow-y-auto">
              ...
              {formData.role === 'admin' && (
                <div className="bg-amber-50 border border-amber-100 rounded-xl p-4 text-amber-800 text-xs font-bold flex items-center gap-3 select-none mb-4">
                      ...
                      <div key={category} className="space-y-2 col-span-1 sm:col-span-2">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                          {categoryModules.map(module => {
                            const isEnabled = formData.role === 'admin' || formData.allowed_modules?.includes(module.key);
                            ...
                            return (
                              ...
                              >
                                ...
                                <button
                                  ...
                                  } ${formData.role === 'admin' ? 'opacity-60 cursor-not-allowed' : ''}`}
                                >
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-slate-100 mt-6">
                ...
                <button
                  ...
                  disabled={submitting}
                  className="px-4 py-2 bg-red-600 hover:bg-red-700 disabled:opacity-50 text-white rounded-xl font-bold text-xs transition-all"
                >
