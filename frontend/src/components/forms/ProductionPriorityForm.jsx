...
export default function ProductionPriorityForm({ onClose, onSuccess, editRecord, moduleType, tabs }) {
  ...
  // Initialize tabData based on passed tabs
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      for (let i = 0; i < tabs.length; i++) {
        ...
        const validRows = currentTab.rows.filter(r => r.job || r.seq || r.weight || r.rtsDate || r.startRun || r.notes);
        if (validRows.length > 0) {
          ...
          if (editRecord && tabs[i] === editRecord.process_type) {
            savePromises.push(priorityAPI.update(editRecord.id, payload));
          } else {
            savePromises.push(priorityAPI.create(payload));
          }
        }
      }
      if (savePromises.length === 0) { alert('Please enter data in at least one row'); setLoading(false); return; }
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-[95vw] lg:max-w-7xl flex flex-col overflow-hidden max-h-[95vh]">
        <div className="flex items-center justify-between px-8 py-5 border-b border-slate-300 bg-white">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-xl font-bold text-slate-900">{editRecord ? 'Edit Priority' : `New ${moduleType} Priority`}</h3>
              <p className="text-xs text-slate-500 mt-0.5">Define priorities for different {moduleType.toLowerCase()} processes</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-6">
          <div className="flex flex-col lg:flex-row gap-6">
            <div className="w-full lg:w-72 shrink-0">
              ...
              <select 
                ...
                className="w-full px-4 py-3 rounded-xl border border-slate-300 bg-white text-sm font-bold text-slate-700 outline-none focus:border-amber-400 focus:ring-4 focus:ring-amber-500/5 transition-all disabled:opacity-50"
              >
            ...
            <div className="flex-1">
              ...
              <div className="flex p-1 bg-white rounded-2xl w-full">
                {tabs.map((tab, idx) => (
                  <button
                    key={tab}
                    disabled={loading}
          ...
          <div className="space-y-6 animate-fade-in" key={activeTab}>
            <div className="flex items-end justify-between gap-4 p-4 bg-slate-50 rounded-2xl border border-slate-300">
              <div className="flex items-center gap-4">
                <div className="bg-white px-4 py-2.5 rounded-xl border border-slate-300 flex items-center gap-3">
                      ...
                      <tr key={index} className="hover:bg-white transition-colors">
                        ...
                        <td className="p-1.5">
                          <div className="relative group">
                            <input type="date" value={row.rtsDate} onChange={(e) => updateRow(index, 'rtsDate', e.target.value)} disabled={loading} className="w-full px-2 py-2 rounded-lg border border-slate-300 focus:border-amber-400 outline-none bg-transparent text-[11px] text-slate-600 appearance-none opacity-0 absolute inset-0 z-10 cursor-pointer" />
                            <div className="w-full px-2 py-2 rounded-lg border border-slate-300 bg-white text-[11px] text-slate-600 flex items-center justify-between">{formatDate(row.rtsDate)}<Calendar className="w-3 h-3 text-slate-400" /></div>
                          </div>
                        </td>
                        <td className="p-1.5"><div className="w-full py-2 bg-amber-50/50 text-amber-700 font-bold text-center text-xs rounded-lg">{row.runDays || '-'}</div></td>
                        <td className="p-1.5">
                          <div className="relative group">
                            <input type="date" value={row.startRun} onChange={(e) => updateRow(index, 'startRun', e.target.value)} disabled={loading} className="w-full px-2 py-2 rounded-lg border border-slate-300 focus:border-amber-400 outline-none bg-transparent text-[11px] text-slate-600 appearance-none opacity-0 absolute inset-0 z-10 cursor-pointer" />
                            <div className="w-full px-2 py-2 rounded-lg border border-slate-300 bg-white text-[11px] text-slate-700 font-bold flex items-center justify-between">{formatDate(row.startRun)}<Calendar className="w-3 h-3 text-slate-400" /></div>
                          </div>
                        </td>
                        <td className="p-1.5"><div className="w-full py-2 bg-green-50 text-green-700 font-bold text-center text-xs rounded-lg">{formatDate(row.completeRunDate)}</div></td>
                        <td className="p-1.5"><input value={row.notes} onChange={(e) => updateRow(index, 'notes', e.target.value)} disabled={loading} className="w-full px-2 py-2 rounded-lg border border-slate-300 focus:border-amber-400 outline-none bg-transparent text-xs truncate" placeholder="..." /></td>
                        <td className="p-1.5 text-center">
                          <button onClick={() => removeRow(index)} disabled={tabData[activeTab].rows.length === 1 || loading} className="p-1.5 text-slate-300 hover:text-red-500 disabled:opacity-0 transition-all"><Trash2 className="w-3.5 h-3.5" /></button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
        ...
        <div className="flex items-center justify-between px-8 py-5 border-t border-slate-300 bg-white">
          <div className="flex gap-2">
            <button onClick={handlePrev} disabled={activeTab === 0 || loading} className="px-4 py-2 rounded-xl border border-slate-300 text-xs font-bold text-slate-600 hover:bg-white disabled:opacity-50 flex items-center gap-2">
          ...
          <div className="flex gap-3">
            <button onClick={onClose} disabled={loading} className="px-6 py-2 rounded-xl border border-slate-300 text-sm font-bold text-slate-600 hover:bg-white transition-all">Cancel</button>
            {isLastTab && (
