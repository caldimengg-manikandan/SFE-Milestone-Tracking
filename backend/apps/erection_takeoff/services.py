...
def compute_erection_totals(erection_takeoff, rate_config, bid_summary):
    ...
    proj_info = project.estimation_data.get("projectInfo", {}) if (project and project.estimation_data) else {}
    erection_type = proj_info.get("erectionType", "inhouse")

    if erection_type == "outsourced":
        freight_out_trucks = Decimal(str(erection_takeoff.freight_out_trucks or 0))
        joist_deck_tons = Decimal(str(erection_takeoff.joist_deck_tons or 0))
    else:
    ...
    # Travel & Per Diem
    if erection_type == "outsourced":
        travel_hours = Decimal(str(erection_takeoff.travel_hours or 0))
    else:
